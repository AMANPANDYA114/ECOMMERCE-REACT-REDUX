...
const Cart = ({ id, name, qty, price, img }) => {
    ...
    <>
      <div className={`fixed right-0 top-0 w-full lg:w-[20vw] h-full p-5 bg-white mb-3  ${activecart ? 'translate-x-0' : 'translate-x-full'} transition-all duration-500 z-50`}>
        ...
        <div className='flex justify-between items-center my-3'>
          ...
          <IoMdCloseCircle onClick={() => setactiveCart(!activecart)} className="border-2 border-gray-600 text-gray-600 font-bold p-2 text-3xl rounded-md hover:text-red-300 hover:border-red-300 cursor-pointer" />
        </div>
...
<FaShoppingCart
  ...
  className={`rounded-full bg-white shadow-md text-5xl p-3 fixed bottom-4 right-4 ${
    totalQty > 0 ? "animate-bounce delay-500 transition-all animate__heartBeat" : ""
  }`}
/>
